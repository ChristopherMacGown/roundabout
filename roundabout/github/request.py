...
def gh_request(username=None, password=None, oauth_token=None):
    yield Request(username, password, oauth_token)
...
class Request(object):
    ...
    def post(self, url, encode=True, **kw):
        if encode:
            post_data = json.dumps(kw)
        else:
           post_data = urllib.urlencode(kw)
        req = HTTPMethodRequest('POST', url, post_data)
        req = self._authenticate(req)
        return self._opener.open(req)
